...
// flattening the single top-level folder most release archives wrap
// their files in (e.g. "llama-b10797/llama-server") so the extracted
// path is stable across build-number bumps - a caller never needs to
// know the archive's internal layout, only the destination directory.
 ...
 * archive (e.g. a platform's separate runtime package) into the same
...
export async function extractArchive(archivePath: string, destDir: string): Promise<void> {
  ...
  try {
    ...
    // If an archive doesn't have exactly one top-level directory, merge
    // its entries directly instead of guessing which one is "the" wrapper.
    const sourceDir =
