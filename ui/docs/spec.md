...
**Note added at `ui-v0.1.0` (2026-09-20):** this is `@maipai/ui`'s own
design specification, approved by the owner on 2026-09-19 while it was
still being built as the Stack's own console. The Stack was refocused
the next day into Home's headless engine layer with no console of its
own, so this document's worked examples (Overview, Models, the machine
selector, the footer, system pulse) are screens of a product that no
longer exists; only the six reference images and the patterns below -
the shell, the things page and table, the browser and pane, the
property panel, the settings workspace, and the phone rows and cards -
travel forward, since those are what this kit actually ships. The body
below is the supplied document unedited; see "What is Stack-only and
not in the kit" at the end for what didn't move.
...
Added at `ui-v0.1.0` (2026-09-20). This document's worked examples name
several Stack-console-specific screens and widgets that were real when
this specification was approved but have no product to belong to since
the Stack's refocus; none of them shipped in `@maipai/ui`:

- The machine/stack selector's own data (switching between multiple
  Stack instances) - its chrome (a header dropdown with a status dot)
  did ship, generalized, as `HeaderPicker`.
- The system pulse widget (CPU/GPU/memory telemetry in the sidebar
  footer).
- The Live page, the Try-it/Tester studio, the model Library and
  showroom.
- The Clients and Keys pages (operator-scoped API keys) - the Stack no
