...
import type { FormNode, ListNode, SectionNode, WidgetCardNode, WidgetRowNode } from "@/kit/schema/types";
import type { WidgetDescriptor, WidgetData } from "@/kit/widgets";
...
describe("NodeRenderer > form", () => {
  const formNode: FormNode = {
    type: "form",
    fields: [{ name: "name", selector: "text", placeholder: "Name" }],
    on_submit: { call: { target: "/api/test-items/create" } },
  };

  test("stays in 'Working…' until the dispatched action actually resolves, then re-enables", async () => {
    const original = globalThis.fetch;
    ...
    try {
      const { getByRole, findByRole } = renderNode(formNode);
      fireEvent.click(getByRole("button", { name: "Submit" }));
      // Still mid-flight: the fetch this submit fired hasn't resolved
      // yet, so the button must still read "Working…" and stay
      // disabled - a synchronous setSubmitting(false) right after
      // dispatch() would have already cleared this by now.
      await findByRole("button", { name: "Working…" });
