...
describe("groupSettings", () => {
  test("groups a basic key under its lives_in section", () => {
    const groups = groupSettings([key()], [value()], "household", "home");
  ...
  test("three or more advanced keys in one section fold", () => {
    ...
    const groups = groupSettings(keys, values, "household", "home");
  ...
  test("a key not honoured by the calling product is filtered out", () => {
    const groups = groupSettings(
  ...
  test("honouredBy is the caller's own product, not a hardcoded one - the same registry renders differently for two different products", () => {
    // "bot", not "home": whatever the spec currently names is real proof
    // the parameter isn't secretly still hardcoded to one product.
    const registry = [key({ key: "shared.thing", honoured_by: ["bot"] })];
    const values = [value({ key: "shared.thing" })];
    expect(groupSettings(registry, values, "household", "home")).toHaveLength(0);
    expect(groupSettings(registry, values, "household", "bot")).toHaveLength(1);
  });

  test("a key from a different scope kind is filtered out", () => {
    const groups = groupSettings([key({ scope: "person" })], [value()], "household", "home");
    expect(groups).toHaveLength(0);
  });

  test("a registry key with no resolved value is skipped, not fabricated", () => {
    const groups = groupSettings([key()], [], "household", "home");
    expect(groups).toHaveLength(0);
  });
});
