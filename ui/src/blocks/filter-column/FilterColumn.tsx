...
export function FilterColumn({ search, groups, onClear, collapsed = false, onCollapsedChange }: FilterColumnProps) {
  ...
  return <aside className="w-60 shrink-0 rounded-xl border bg-[var(--surface-card)] p-4" data-testid="filter-column"><div className="flex items-center justify-between gap-2"><p className="text-sm font-medium">Filters</p>{onCollapsedChange && <Button type="button" size="icon" variant="ghost" aria-label="Collapse filters" title="Collapse filters" onClick={() => onCollapsedChange(true)}><ChevronLeft className="size-4" /></Button>}</div><div className="relative mt-3"><Search className="pointer-events-none absolute top-4 left-3 size-4 text-muted-foreground" /><Input aria-label="Search filters" value={search.value} placeholder={search.placeholder ?? "Search"} onChange={(event) => handleSearch(event, search.onChange)} className="pl-9" /></div><div className="mt-4 space-y-3">{groups.map((group) => { const open = openGroups.has(group.id); return <Collapsible key={group.id} open={open} onOpenChange={(next) => setOpenGroups((current) => { const updated = new Set(current); if (next) updated.add(group.id); else updated.delete(group.id); return updated; })}><div className="border-t pt-3"><CollapsibleTrigger asChild><button type="button" className={cn("flex w-full items-center justify-between py-1 text-left text-sm font-medium", hitArea(3))} aria-label={`${open ? "Collapse" : "Expand"} ${group.title}`}><span>{group.title}</span>{open ? <ChevronDown className="size-4 text-muted-foreground" /> : <ChevronRight className="size-4 text-muted-foreground" />}</button></CollapsibleTrigger><CollapsibleContent className="mt-3 space-y-1">{group.options.map((option) => <label key={option.id} className="flex min-h-12 cursor-pointer items-center gap-2 rounded-md px-1 py-1 text-sm hover:bg-muted"><Checkbox aria-label={option.label} checked={group.selected.has(option.id)} onCheckedChange={(checked) => { const selected = new Set(group.selected); if (checked === true) selected.add(option.id); else selected.delete(option.id); group.onChange(selected); }} /><span className="min-w-0 flex-1 truncate">{option.label}</span><span className="tabular-nums text-xs text-muted-foreground">{option.count}</span></label>)}</CollapsibleContent></div></Collapsible>; })}</div><Button type="button" variant="link" className={cn("mt-4 h-auto px-1 text-sm", hitArea(2))} onClick={onClear}>Clear filters</Button></aside>;
}
