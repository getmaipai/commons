...
function PanelBody({ kind, item, status, actions, primaryActions, facts = [], usedBy = [], tabs, onClose }: Omit<PropertyPanelProps, "open">) {
  ...
  const tabEntries = [{ id: "overview", label: "Overview", icon: "LayoutList" as const, content: tabs.overview }, ...(tabs.insights ? [{ id: "insights", label: "Insights", icon: "activity" as const, content: tabs.insights }] : []), ...(tabs.settings ? [{ id: "settings", label: "Settings", icon: "settings-2" as const, content: tabs.settings }] : [])];
  function runAction(action: PropertyAction) { if (action.destructive) setConfirm(action); else void action.onClick(); }
  const disabledSentences = [...new Set(actions.filter((action) => action.disabled && action.sentence).map((action) => action.sentence!))];
return <TooltipProvider><div className="flex h-full flex-col" data-testid="property-panel"><div className="p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{kind}</p><h2 className="mt-1 line-clamp-2 text-xl font-semibold leading-tight">{panelTitle(item)}</h2></div><div className="flex shrink-0 items-center gap-1"><Badge variant={status.toLowerCase().includes("offline") ? "destructive" : "secondary"}>{status}</Badge><Button size="icon" variant="ghost" aria-label="Close panel" onClick={onClose}>×</Button></div></div><div className="mt-3 flex flex-wrap gap-4">{actions.map((action) => <Tooltip key={action.label}><TooltipTrigger asChild><Button size="icon-sm" variant="ghost" aria-label={action.label} aria-disabled={action.disabled || undefined} disabled={action.disabled} onClick={() => runAction(action)}><ActionIcon name={action.icon} /></Button></TooltipTrigger><TooltipContent>{action.disabled && action.sentence ? `${action.label}: ${action.sentence}` : action.label}</TooltipContent></Tooltip>)}</div>{disabledSentences.length > 0 && <div className="mt-2 space-y-1">{disabledSentences.map((sentence) => <p key={sentence} className="text-xs text-muted-foreground">{sentence}</p>)}</div>}{facts.length > 0 && <div className="mt-3"><PanelSection title="State" icon="activity"><div className="grid gap-2 text-sm sm:grid-cols-3">{facts.slice(0, 3).map((fact) => <div key={fact.label}><p className="text-xs text-muted-foreground">{fact.label}</p><p className="mt-1 font-medium tabular-nums">{fact.value}</p></div>)}</div></PanelSection></div>}{primaryActions && <div className="mt-3 grid grid-cols-2 gap-2">{primaryActions.map((action) => <Button key={action.label} variant="outline" disabled={action.disabled} onClick={() => runAction(action)}>{action.label}</Button>)}</div>}{confirm && <Card className="mt-3 border-destructive"><CardContent className="space-y-3 p-3"><p className="text-sm">{confirm.confirmLabel ?? `${confirm.label} ${panelTitle(item)}?`}</p><div className="flex justify-end gap-4"><Button size="sm" variant="outline" onClick={() => setConfirm(null)}>Cancel</Button><Button size="sm" variant="destructive" onClick={async () => { await confirm.onClick(); setConfirm(null); }}>Confirm</Button></div></CardContent></Card>}</div><Tabs defaultValue="overview" className="min-h-0 flex-1 overflow-y-auto"><TabsList variant="line" className="w-full justify-start rounded-none border-y px-4 py-2">{tabEntries.map((tab) => { const Icon = getIcon(tab.icon); return <Tooltip key={tab.id}><TooltipTrigger asChild><TabsTrigger value={tab.id} aria-label={tab.label}><Icon className="size-4" /></TabsTrigger></TooltipTrigger><TooltipContent>{tab.label}</TooltipContent></Tooltip>; })}</TabsList>{tabEntries.map((tab) => <TabsContent key={tab.id} value={tab.id} className="p-3"><PanelSection title={tab.id === "overview" ? "Configuration" : tab.id === "insights" ? "Usage" : "Settings"} icon={tab.id === "insights" ? "activity" : tab.id === "settings" ? "settings-2" : "sliders-horizontal"}>{tab.content}</PanelSection></TabsContent>)}</Tabs>{usedBy.length > 0 && <div className="p-3"><PanelSection title="Usage" icon="server"><UsedBy items={usedBy} /></PanelSection></div>}</div></TooltipProvider>;
}
